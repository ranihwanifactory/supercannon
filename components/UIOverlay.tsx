...
import { GameState, UpgradeStats, GameStats, UserProfile, LeaderboardEntry, Castle } from '../types';
...
const UIOverlay: React.FC<UIOverlayProps> = ({
  ...
  // Use state to force re-render when reading mutable castle ref in loops
  ...
  // Fetch Leaderboard based on Level now (conceptually, though DB structure assumes score sort, we can sort manually or adjust query)
  // For simplicity, we stick to fetching and sorting here.
  useEffect(() => {
    if (gameState === GameState.LEADERBOARD || gameState === GameState.LANDED || gameState === GameState.LEVEL_CLEARED) {
      const fetchLeaderboard = async () => {
        // Query by level primarily
        const q = query(ref(db, DB_LEADERBOARD_PATH), orderByChild('level'), limitToLast(20));
        const snapshot = await get(q);
        const data: LeaderboardEntry[] = [];
        snapshot.forEach((child) => {
          data.push(child.val());
        });
        setLeaderboard(data.reverse()); // Highest level first
      };
      fetchLeaderboard();
    }
  }, [gameState]);
    ...
    <div className="absolute inset-0 pointer-events-none flex flex-col justify-between p-4 z-10 font-sans">
      ...
      <div className="flex justify-between items-start pointer-events-auto">
        <div className="flex flex-col gap-2">
            <div className="bg-slate-900/80 backdrop-blur rounded-lg p-3 border border-purple-500/30 text-yellow-400 font-bold shadow-lg w-32">
            💎 {upgrades.money.toLocaleString()}
            </div>
            
            {/* Target Info */}
            <div className="bg-red-900/80 backdrop-blur rounded-lg p-3 border border-red-500/30 text-white shadow-lg w-48">
                <div className="text-xs font-bold text-red-300 mb-1">TARGET: STAGE {currentLevel} CASTLE</div>
                <div className="h-3 w-full bg-black/50 rounded-full overflow-hidden">
                    <div className="h-full bg-red-500 transition-all duration-200" style={{width: `${(castleRef.current.currentHealth / castleRef.current.maxHealth) * 100}%`}}></div>
                </div>
                <div className="text-right text-[10px] mt-1 text-red-200">
                    {Math.floor(castleRef.current.currentHealth)} / {Math.floor(castleRef.current.maxHealth)} HP
                </div>
            </div>
        </div>
        ...
        <div className="flex gap-2">
            ...
            <div onClick={() => setGameState(GameState.LEADERBOARD)} className="cursor-pointer bg-slate-900/80 backdrop-blur rounded-lg p-3 border border-purple-500/30 text-white font-bold shadow-lg text-center hover:bg-slate-800">
                <div>STAGE {currentLevel}</div>
                <div className="text-xs text-purple-300">MAX: STAGE {gameStats.maxLevel}</div>
            </div>
      ...
      {gameState === GameState.MENU && (
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-center pointer-events-none animate-pulse w-full">
          ...
          <p className="text-purple-200 text-lg mt-4 font-bold bg-slate-900/50 inline-block px-4 py-2 rounded backdrop-blur-sm">
             STAGE {currentLevel} : DESTROY THE CASTLE!
          </p>
        </div>
      )}
      ...
      {gameState === GameState.AIMING && (
         <div className="absolute top-1/3 left-1/2 transform -translate-x-1/2 text-white/50 text-sm font-bold tracking-widest pointer-events-none">
            AIMING - DISTANCE: {castleRef.current.x}m
         </div>
      )}
          ...
          <div className="absolute inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center pointer-events-auto z-50">
             <div className="bg-slate-800 p-6 rounded-2xl border border-purple-500/50 shadow-2xl max-w-sm w-full relative">
                ...
                <form onSubmit={handleEmailAuth} className="space-y-3">
                    <input type="email" placeholder="이메일" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full p-3 bg-slate-700 rounded border border-slate-600 text-white focus:border-purple-500 outline-none" />
                    <input type="password" placeholder="비밀번호" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full p-3 bg-slate-700 rounded border border-slate-600 text-white focus:border-purple-500 outline-none" />
                    {authError && <p className="text-red-400 text-xs">{authError}</p>}
                    <button type="submit" className="w-full bg-purple-600 hover:bg-purple-500 text-white font-bold py-3 rounded-lg transition">{authMode === 'login' ? '로그인' : '회원가입'}</button>
                </form>
                <div className="text-center mt-4 text-sm text-slate-400">
                    <button onClick={() => setAuthMode(authMode === 'login' ? 'signup' : 'login')} className="text-purple-400 hover:underline">{authMode === 'login' ? '회원가입' : '로그인'}</button>
                </div>
             </div>
          </div>
      )}
      ...
      {gameState === GameState.LEADERBOARD && (
          <div className="absolute inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center pointer-events-auto z-50">
             <div className="bg-slate-800 p-6 rounded-2xl border border-purple-500/50 shadow-2xl max-w-sm w-full max-h-[80vh] flex flex-col relative">
                <button onClick={() => setGameState(GameState.MENU)} className="absolute top-4 right-4 text-slate-400 hover:text-white">✕</button>
                <h2 className="text-2xl font-bold text-center text-white mb-4">🏆 RANKING</h2>
                <div className="flex-1 overflow-y-auto space-y-2 pr-1 custom-scrollbar">
                    {leaderboard.length === 0 ? <p className="text-center text-slate-500 py-10">Loading...</p> : leaderboard.map((entry, index) => (
                        <div key={index} className={`flex items-center p-3 rounded-lg ${entry.uid === user?.uid ? 'bg-purple-900/50 border border-purple-500' : 'bg-slate-700'}`}>
                            ...
                            <div className="flex-1">
                                ...
                                <div className="text-xs text-slate-400">Score: {entry.score}</div>
                            </div>
                            <div className="text-yellow-400 font-black text-lg">Lv.{entry.level || 1}</div>
                        </div>
                    ))}
                </div>
             </div>
      ...
      {/* Round End / Level Clear Modal */}
      {(gameState === GameState.LANDED || gameState === GameState.LEVEL_CLEARED) && (
        <div className="absolute inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center pointer-events-auto animate-fade-in">
          <div className={`bg-slate-800 p-6 rounded-2xl border-4 ${gameState === GameState.LEVEL_CLEARED ? 'border-yellow-400 shadow-[0_0_50px_rgba(250,204,21,0.5)]' : 'border-slate-600'} max-w-sm w-full relative`}>
            
            {gameState === GameState.LEVEL_CLEARED ? (
                <>
                    <h2 className="text-3xl font-black text-center text-yellow-400 mb-2 animate-bounce">VICTORY!</h2>
                    <div className="text-center mb-6">
                        <p className="text-white text-lg">STAGE {currentLevel} CLEARED</p>
                        <p className="text-purple-300 text-sm">Castle Destroyed!</p>
                    </div>
                </>
            ) : (
                <>
                    <h2 className="text-2xl font-bold text-center text-white mb-2">MISSED?</h2>
                    <div className="text-center mb-6">
                        <span className="text-5xl font-black text-transparent bg-clip-text bg-gradient-to-r from-gray-400 to-white drop-shadow-lg">{distance}m</span>
                        <div className="mt-2 text-red-400 font-bold">
                             Castle HP: {Math.floor(castleRef.current.currentHealth)} left
                        </div>
                    </div>
                </>
            )}
            ...
            <div className="space-y-3 mb-6">
              ...
              <div className="bg-slate-700/50 p-3 rounded flex justify-between items-center border border-slate-600">
                <div>
                  <div className="text-xs text-slate-300 font-bold">공기역학</div>
                  <div className="h-2 w-24 bg-slate-900 rounded-full mt-1 overflow-hidden">
               ...
               <div className="bg-slate-700/50 p-3 rounded flex justify-between items-center border border-slate-600">
                <div>
                  <div className="text-xs text-slate-300 font-bold">탄성</div>
                  <div className="h-2 w-24 bg-slate-900 rounded-full mt-1 overflow-hidden">
            ...
            <div className="flex gap-2">
                {gameState === GameState.LEVEL_CLEARED ? (
                    <button 
                      onClick={onNextLevel}
                      className="flex-1 py-3 bg-green-600 hover:bg-green-500 rounded-xl font-bold text-white text-lg shadow-lg active:scale-95 transition-transform"
                    >
                      NEXT LEVEL ⚔️
                    </button>
                ) : (
